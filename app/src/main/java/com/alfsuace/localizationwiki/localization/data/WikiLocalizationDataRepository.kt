...
import com.alfsuace.localizationwiki.localization.data.local.db.WikiLocalizationDbSource
...
class WikiLocalizationDataRepository(
    private val wikiLocalizationDbSource: WikiLocalizationDbSource,
    ...
    override suspend fun getNearbyWikis(lat: Double, lon: Double): Result<List<WikiLocalization>> {
        return wikiLocalizationDbSource.getAllWikis().onFailure { localError ->
            if (localError is ErrorApp.CacheExpiredErrorApp || localError is ErrorApp.DataErrorApp) {
                wikiLocalizationApiRemoteSource.getNearbyWikiLocalizations(lat, lon)
                    .onSuccess { remoteWikis ->
                        wikiLocalizationDbSource.deleteAllWikis()
                        wikiLocalizationDbSource.saveWikis(remoteWikis)
                    }
